...
import { recordTransfer } from "@/app/lib/ledger-transfer-service";
...
function errorResponse(error: unknown) {
  ...
  console.error("finance_v1_transfer_failed", error instanceof Error ? error.name : "unknown");
  return NextResponse.json(
...
export async function POST(request: Request) {
  try {
    ...
    const entry = await recordTransfer(parseTransferCommand(body));
